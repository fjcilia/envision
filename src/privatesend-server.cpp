...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Envision related functionality
...
/*
    ...
    allow endless transaction that would bloat Envision and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Envision specific functionality
    ...
    RenameThread("envision-ps-server");
