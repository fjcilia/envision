 ...
 *
 *   // network
 *   CEnvisionNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.envision.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CEnvisionNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CEnvisionNetwork(UniValue objIn)
    {
...
// // can be under: EnvisionNetwork
// //   -- signature requirements : Key1(User)
// class CEnvisionNetworkVariable : public CGovernanceObject
// {
// private:
