...
const char * const BITCOIN_CONF_FILENAME = "envision.conf";
const char * const BITCOIN_PID_FILENAME = "envisiond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "envision" is a composite category enabling all Envision-related debug output
            if(ptrCategory->count(string("envision"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EnvisionCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EnvisionCore
    // Mac: ~/Library/Application Support/EnvisionCore
    // Unix: ~/.envisioncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EnvisionCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EnvisionCore";
#else
    // Unix
    return pathRet / ".envisioncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty envision.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override envision.conf
